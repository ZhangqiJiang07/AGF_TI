...
B_n_gamma_old = spdiags(temp_B_n', 0, num_samples, num_samples)./gamma;
B_m_gamma_old = spdiags(ones(num_anchors, 1), 0, num_anchors, num_anchors)./gamma;
B_n_gamma = temp_B_n./gamma;
B_m_gamma = ones(1, num_anchors)./gamma;
...
Lambda_inv_1_2 = sum(unified_P, 1).^(-1/2);
Y = zeros(num_samples, num_class);
for i = 1:num_labeled
    Y(i, Ll(i)) = 1;
end
I_n = ones(1, num_samples);
I_m = ones(1, num_anchors);
...
temp_inv = inv(diag(L22) - L21 .* L11_inv * L12);
